...
def make_notes(cd_element):
    cd_notes = momapy.celldesigner.io.celldesigner._parsing.get_notes(cd_element)
...
def make_annotations_from_element(cd_element):
    cd_rdf = momapy.celldesigner.io.celldesigner._parsing.get_rdf(cd_element)
...
def make_annotations_from_notes(cd_notes):
    cd_rdf = momapy.celldesigner.io.celldesigner._parsing.get_rdf_from_notes(cd_notes)
    if cd_rdf is not None:
        annotations = make_annotations(cd_rdf)
    else:
        annotations = []
...
def make_compartment(cd_compartment, model):
    ...
    model_element.name = momapy.celldesigner.io.celldesigner._parsing.make_name(
