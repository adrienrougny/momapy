...
def set_layout_size_and_position(cd_model, layout):
    layout.width = float(momapy.celldesigner.io.celldesigner._parsing.get_width(cd_model))
    layout.height = float(momapy.celldesigner.io.celldesigner._parsing.get_height(cd_model))
...
def make_species(cd_species_alias, layout, layout_element_cls, name, homomultimer, active):
    layout_element = layout.new_element(layout_element_cls)
    layout_element.id_ = cd_species_alias.get("id")
    cd_x, cd_y, cd_w, cd_h = (
        momapy.celldesigner.io.celldesigner._parsing.get_bounds(cd_species_alias)
    )
...
def make_species_structural_state(cd_species_structural_state, layout, super_layout_element):
    layout_element = layout.new_element(
...
def make_segments_non_t_shape(cd_reaction, cd_id_to_layout_element):
    cd_base_reactants = (
        momapy.celldesigner.io.celldesigner._parsing.get_base_reactants(cd_reaction)
    )
    cd_base_products = (
        momapy.celldesigner.io.celldesigner._parsing.get_base_products(cd_reaction)
    )
...
def make_reactant_from_base(
    cd_base_reactant, n_cd_base_reactant, cd_reaction, layout, cd_id_to_layout_element, super_layout_element
):
    layout_element = layout.new_element(
        momapy.celldesigner.core.ConsumptionLayout
    )
    cd_edit_points = momapy.celldesigner.io.celldesigner._parsing.get_edit_points_from_reaction(
        cd_reaction
    )
...
def make_reactant_from_link(cd_reactant_link, layout, cd_id_to_layout_element, super_layout_element):
    layout_element = layout.new_element(
        momapy.celldesigner.core.ConsumptionLayout
    )
    species_layout_element = cd_id_to_layout_element[
...
def make_product_from_base(
    cd_base_product, n_cd_base_product, cd_reaction, layout, cd_id_to_layout_element, super_layout_element
):
    layout_element = layout.new_element(
...
def make_product_from_link(cd_product_link, layout, cd_id_to_layout_element, super_layout_element):
    layout_element = layout.new_element(
        momapy.celldesigner.core.ProductionLayout
    )
    species_layout_element = cd_id_to_layout_element[
...
def make_modifier(
    cd_reaction_modification, layout, layout_element_cls, source_layout_element, super_layout_element, has_boolean_input
):
    layout_element = layout.new_element(layout_element_cls)
    ...
    if not has_boolean_input:
        ...
        source_anchor_name = "center"
        edit_points = edit_points[:-1]  # last point is the position of the logic gate
...
def make_modulation(
    ...
    if hasattr(cd_base_product, "linkAnchor"):
        target_anchor_name = momapy.celldesigner.io.celldesigner._parsing.get_anchor_name_for_frame(
            cd_base_product
        )
    else:
